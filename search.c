...
static int corrected_eval(Position *board, int raw_eval, int ply)
{
    ...
    if (ply > 0 && ply - 1 < MAX_SEARCH_PLY && cont_stack[ply - 1].valid)
    {
        ...
        correction += cont_corrhist[board->turn][pp][pt];
    }
...
static void update_corrhist(Position *board, int ply, int depth, int static_eval, int best_score)
{
    ...
    *npc += bonus - *npc * abs(bonus) / CORRHIST_LIMIT;
    ...
    if (ply > 0 && ply - 1 < MAX_SEARCH_PLY && cont_stack[ply - 1].valid)
    {
        int pp = cont_stack[ply - 1].piece;
        int pt = cont_stack[ply - 1].to;
        int *cc = &cont_corrhist[side][pp][pt];
                    ...
                    stopConditions *stop, PVLine *pv, SearchStack *stack)
{
    ...
    if (!stop->stop && stack->excluded_move == 0 && !in_check)
        update_corrhist(board, ply, depth, static_eval, best_score);
