...
static int nnue_load(void) {
	...
	memcpy(nnue_featureWeights, data + offset,
		   sizeof(nnue_featureWeights)); // Copy to static array
	offset +=
		sizeof(nnue_featureWeights); // Moves offset from weights to biases

	memcpy(nnue_hiddenBiases, data + offset,
		   sizeof(nnue_hiddenBiases)); // Copy to static array
	offset +=
		sizeof(nnue_hiddenBiases); // Moves offset from biases to output weights

	memcpy(nnue_outputWeights, data + offset,
		   sizeof(nnue_outputWeights));	  // Copy to static array
	offset += sizeof(nnue_outputWeights); // Moves offset from output weights to
...
static inline int nnue_inputIndex(int persp, int pieceIsOwn, int internal_piece,
	...
	int relSq =
		(persp == 0) ? NNUE_FLIP(sq) : sq; // Flip relative perspective because
										   // internal representation is flipped
	if (mirror)
		relSq ^= 7; // flips square for mirroring
...
static void nnue_touchPiece(NnueAccumulator *acc, int internal_piece, int color,
							int sq, int sign) {
	for (int persp = 0; persp < 2; persp++) {
		int pieceIsOwn = (color == persp);
		int input_index =
			nnue_inputIndex(persp, pieceIsOwn, internal_piece, sq,
							acc->mirror[persp]); // passes in mirror parameter
		if (sign > 0)
...
static void nnue_touchPiecePerspective(NnueAccumulator *acc, int internal_piece,
									   int color, int sq, int sign, int persp) {
	int pieceIsOwn = (color == persp);
...
static void nnue_buildSide(Position *board, int ownSide, int16_t acc[NNUE_HL],
	...
	for (int internal_piece = 0; internal_piece < 6; internal_piece++) {
		...
		while (bb) {
			int sq = pop_lsb(&bb);
			nnue_addFeature(
				acc, nnue_inputIndex(ownSide, 1, internal_piece, sq, mirror));
		}
		...
		while (bb) {
			int sq = pop_lsb(&bb);
			nnue_addFeature(
				acc, nnue_inputIndex(ownSide, 0, internal_piece, sq, mirror));
		}
	}
}
...
void nnue_refresh(Position *board, int ply) {
	...
	acc->mirror[WHITE] = mirror_or_not(
		board->pieces[KINGNUMBER] &
		board->color[WHITE]); // Sets flag for white to mirror or not
	acc->mirror[BLACK] = mirror_or_not(
		board->pieces[KINGNUMBER] &
		board->color[BLACK]); // Sets flag for black to mirror or not
...
static int nnue_forward(Position *board, int ply) {
	...
	for (int h = 0; h < NNUE_HL; h++) {
		...
		sum += (long long)nnue_screlu(accThem[h]) *
			   nnue_outputWeights[NNUE_HL + h];
	}

	sum = sum / HIDDEN_QUANT_SCALE + nnue_outputBias;
	sum =
		sum * NNUE_SCALE / ((long long)HIDDEN_QUANT_SCALE * NNUE_OUTPUT_SCALE);
