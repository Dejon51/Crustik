...
unsigned long simple_rand(void) {
    ...
    if (state == 0) {  // initialize state on first call
        int x, y;
...
void append64char(char arr[], int value) {
    ...
    for (i = 0; i < 64; i++) {
        if (arr[i] == 98) { // current code only fills empty with non-zero
            arr[i] = value;
            return;
        }
    }
